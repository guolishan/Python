...
# 查询
def get_dpt_info(tk_query, tk_result):
    query_name = tk_query.get()
...
def create_tk():
    ...
    label_name = tkinter.Label(root, text='(回车即可查询)', anchor='w')
    label_name.place(x=420, y=30, width=100, height=30)

    entry_result = tkinter.Text(root, bd=1)
    entry_result.place(x=50, y=90, width=600, height=200)

    entry_name.bind("<Return>", lambda event:get_dpt_info(entry_name, entry_result))
